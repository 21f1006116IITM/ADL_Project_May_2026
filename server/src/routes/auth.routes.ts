...
router.post("/signup", async (req: Request, res: Response) => {
  try {
    ...
    if (role === "couple") {
      if (!name || !partner_name || !location || !phone_number) {
        return res.status(400).json({ error: "name, partner_name, location, and phone_number are required for couple signup" });
      }
    ...
    return res.status(500).json({ error: "Signup failed due to a server error. Please try again." });
  }
});
